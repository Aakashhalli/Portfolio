...
const ProjectCard = ({ 
  ...
  return (
    <motion.div 
      variants={variants} 
      className="project-card group relative grid grid-cols-1 lg:grid-cols-12 gap-8 rounded-2xl overflow-hidden"
    >
      {/* Project Image */}
      <Card className={`col-span-12 lg:col-span-6 ${isEven ? 'lg:order-1' : 'lg:order-2'} overflow-hidden relative h-full border-none shadow-lg`}>
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-black/10 z-10 opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-center justify-center">
          <div className="flex gap-6 translate-y-8 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 delay-100">
            <Button size="icon" className="rounded-full bg-white/20 backdrop-blur-sm border border-white/40 hover:bg-white/30 transform hover:scale-110 transition-all duration-300" asChild>
      ...
      {/* Project Description */}
      <div className={`col-span-12 lg:col-span-6 ${isEven ? 'lg:order-2' : 'lg:order-1'} flex flex-col justify-center`}>
        ...
        <h3 className="text-2xl lg:text-3xl font-bold mb-4 text-primary bg-gradient-to-r from-primary to-secondary bg-clip-text">{project.title}</h3>
